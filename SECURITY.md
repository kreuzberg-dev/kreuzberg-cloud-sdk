...
These SDKs handle **end-user credentials** for the Xberg API. In scope: leakage of API keys or tokens through logs, error messages, or serialized output; TLS or certificate verification that can be silently disabled; unsafe deserialization of API responses; and any default that sends credentials to a host other than the configured endpoint.
